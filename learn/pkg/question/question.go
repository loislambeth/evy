...
func (m *Model) getVerifiedAnswer() (Answer, error) {
	...
	generated := m.Question.ResultOutput()
	for i, choice := range m.AnswerChoices {
		choiceGen := choice.ResultOutput()
...
func (m *Model) buildAnswerChoicesField(block markdown.Block) error {
	...
	for _, item := range list.Items {
		...
		for _, b := range item.(*markdown.Item).Blocks {
			...
			if renderer != nil && found {
				return fmt.Errorf("%w: found second Renderer in one list item", ErrBadMarkdownStructure)
			}
			if renderer != nil {
				found = true
				m.AnswerChoices = append(m.AnswerChoices, renderer)
				m.answerList = block
				m.trackRepBlock(b, renderer)
			}
		}
	}
	if len(m.AnswerChoices) != 0 && len(m.AnswerChoices) != len(list.Items) {
		return fmt.Errorf("%w: found %d output rendering items, expected %d (one per list item)", ErrBadMarkdownStructure, len(m.AnswerChoices), len(list.Items))
	}
	return nil
}

func (m *Model) trackRepBlock(b markdown.Block, renderer Renderer) {
	text := toText(b)
